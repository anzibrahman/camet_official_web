...
const configs = {
  ...
  addons: { label: 'Tally Add-ons', singular: 'Add-on', defaults: { slug: '', title: '', subtitle: '', points: [], isActive: true } },
...
export default function ContentManager() {
  ...
  const edit = (item) => { setEditing(item); setMedia(null); setForm({ ...configs[type].defaults, ...item }); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  ...
  const save = async (event) => {
    ...
    try {
      ...
      if (media) {
        ...
        if (type === 'addons') payload.video = uploaded
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <form onSubmit={save} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="space-y-3">
          ...
          {type === 'addons' && <><input name="title" value={form.title} onChange={change} placeholder="Add-on name" required className="field" /><input name="slug" value={form.slug} onChange={change} placeholder="slug" required className="field" /><input name="subtitle" value={form.subtitle} onChange={change} placeholder="Short description" className="field" /><textarea value={lines(form.points)} onChange={(e) => arrayChange('points', e.target.value)} placeholder="Points — one per line" className="field" rows="6" /></>}
          ...
          <label className="block text-sm font-medium text-slate-700">{mediaLabel}<input type="file" accept="image/*,video/*" onChange={(e) => setMedia(e.target.files?.[0] || null)} className="mt-1 block w-full text-sm" /></label>
